...
import { getAccountsByCustId, withdraw } from "../api/customerApi";
...
export default function Withdraw() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setAccounts(list);
        if (list.length) {
          setAccNo(list[0].accountNo ?? list[0].accNo);
        }
      } catch (err) {
        console.error(err);
  ...
  async function handleWithdraw(e) {
    ...
    try {
      setLoading(true);
      const res = await withdraw({ accNo, amount: Number(amount) });
  ...
  return (
    <CustomerLayout active="withdraw">
      <div className="max-w-xl bg-slate-900/80 border border-slate-800 rounded-2xl p-6">
        ...
        <p className="text-xs text-slate-400 mb-4">
          Select an account and enter the amount you want to withdraw. Make sure
          you maintain the required minimum balance.
        </p>
